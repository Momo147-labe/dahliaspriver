...
class FraisModal extends StatefulWidget {
  final bool isEdit;
  final Map<String, dynamic> frais;
  final List<Map<String, dynamic>> classes;
  final VoidCallback onSave;
  final VoidCallback onClose;
  final bool allowMultipleClasses;
  ...
  @override
  State<FraisModal> createState() => _FraisModalState();
}
...
class _FraisModalState extends State<FraisModal> {
  ...
  void initState() {
    super.initState();
    _loadAnneesScolaires();
    _initializeControllers();
    _initializeSelectedClasses();
  }

  void _initializeSelectedClasses() {
    if (widget.allowMultipleClasses) {
      _selectedClassIds = [];
    } else if (widget.frais['classe_id'] != null) {
      _selectedClassIds = [widget.frais['classe_id']];
    }
  }

  Future<void> _loadAnneesScolaires() async {
    try {
      ...
      if (activeAnnee != null) {
        setState(() {
          ...
          if (!widget.isEdit) {
            widget.frais['annee_scolaire_id'] = activeAnnee['id'];
          }
        });
      }
    } catch (e) {
  ...
  void _initializeControllers() {
    ...
    _parseDate(widget.frais['date_limite_t1'], (d, s) {
      _dateLimite1 = d;
      _dateLimite1Controller.text = s;
    });
    _parseDate(widget.frais['date_limite_t2'], (d, s) {
      _dateLimite2 = d;
      _dateLimite2Controller.text = s;
    });
    _parseDate(widget.frais['date_limite_t3'], (d, s) {
      _dateLimite3 = d;
      _dateLimite3Controller.text = s;
    });
  }

  void _parseDate(String? dateStr, Function(DateTime, String) onParsed) {
    if (dateStr != null && dateStr.isNotEmpty) {
      try {
        final date = _dateFormat.parse(dateStr);
        onParsed(date, _dateFormat.format(date));
      } catch (e) {
        // Fallback or ignore
      }
    }
  ...
  void _updateFraisData() {
    ...
    widget.frais['date_limite_t1'] = _dateLimite1Controller.text;
    widget.frais['date_limite_t2'] = _dateLimite2Controller.text;
    widget.frais['date_limite_t3'] = _dateLimite3Controller.text;
    widget.frais['montant_total'] = _montantTotal;
  }

  Future<void> _saveMultipleClassesFrais() async {
    try {
      final fraisData = {
      ...
      await DatabaseHelper.instance.createFraisForMultipleClasses(
        _selectedClassIds,
        widget.frais['annee_scolaire_id'],
        fraisData,
      );
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: widget.isEdit
              ...
              : [AppTheme.primaryColor, Colors.orange.shade600],
        ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              widget.isEdit ? Symbols.edit : Symbols.payments,
              color: Colors.white,
              size: 28,
            ),
          ),
  ...
  Widget _buildSingleClassSelection(bool isDark) {
    return Row(
      children: [
        Expanded(
  ...
  Widget _buildMultiClassSelection(bool isDark) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildTextField(
